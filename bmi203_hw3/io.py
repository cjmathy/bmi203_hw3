    ...
    with open(filepath, "r") as f:
        ...
        for line in f:
            if line[0:3] != 'TER':
                ...
                #Added this conditional because some active site pdb files contain duplicate residues. It checks whether any of the current residues in the active site have the same number as the residue about to be added. Residues are only added to the active site if this is not the case.
                if all(residue.number != any_residue.number for any_residue in active_site.residues):
                    active_site.residues.append(residue)

    return active_site


def write_P_clustering(filename, clusters,s):
    """
...
def write_H_clustering(filename, clusterings):
    """
    ...
    for i in clusterings: #I returned clusterings as a dictionary, rather than a list

        cluster = clusterings[i]
        out.write("\nCluster %d\n------------\n" % i)
        for j in cluster:
            out.write(str(cluster[j])+"\n")
